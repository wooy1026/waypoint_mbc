...
class move_limo:
    def __init__(self):
        ...
        self.lx = reading_csv.iloc[:, 0].tolist() #첫번째 열 가져와서 변수리스트 저장
        self.ly = reading_csv.iloc[:, 1].tolist() #두번째 열 가져와서 변수리스트 저장


    def utm_x_callback(self, data):
        self.utm_x_raw = data.data
        self.utm_x = self.utm_x_raw + 0.4 * math.sin(self.yaw * math.pi/180) #base_offset_x

    def utm_y_callback(self, data):
        self.utm_y_raw = data.data
        self.utm_y = self.utm_y_raw + 0.4 * math.cos(self.yaw * math.pi/180) #base_offset_y

    def cal_heading(self): #self.utm 좌표로 자신의 헤딩값을 계산
        if self.prev_x is not None and self.prev_y is not None:
            dx = self.utm_x - self.prev_x
            dy = self.utm_y - self.prev_y
            cal_angle = math.atan2(dy, dx) * 180 / math.pi  # 각도를 라디안에서 도(degree)로 변환
            if cal_angle < 0 :
                cal_angle += 360
                
        # 현재 좌표를 이전 좌표로 저장
        self.prev_x = self.utm_x
        self.prev_y = self.utm_y
        return cal_angle
