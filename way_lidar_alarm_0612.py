...
class move_limo:
    def __init__(self):
        ...
        #obstacle_init
        self.dist_obstacle = 3 # 장애물 거리 초기값
        self.x_list = [] # 장애물 x좌표 리스트
        self.y_list = [] # 장애물 y좌표 리스트
        self.last_alarm_time = 0  # 마지막 알람 시간
        ...
        #카카오톡 알람 
        self.service_key = "d7fbc8a68b786c91e039530cc7d04832" # 성환 서비스 키
        # self.service_key = "d3478140dfe991d07a51a6f6b53e54b0" # 우영 서비스 키
        self.token = "IKFEaXHofDXdE1VwJ2zVCx_B6pgQJkkZAAAAAQopyV4AAAGQB0gi5Fv0-avl6D9k"
    ...
    def scan_callback(self, data):
        ...
        for i, n in enumerate(data.ranges):
            angle = data.angle_min + data.angle_increment * i  # -90도 ~ 90도
            angle_degree = angle * 180 / math.pi # -90도 ~ 90도
            x = -n * math.cos(angle)
            y = n * math.sin(angle) 
            if y > -0.25 and y < 0.25 and x > 0:
                self.x_list.append(x) # 장애물의 x 거리만 거리 저장
                self.y_list.append(y) # 장애물의 y 거리만 거리 저장
        if self.x_list:
            self.dist_obstacle = min(self.x_list) # 장애물의 최소 거리 저장
        else:
    ...
    def utm_x_callback(self, data):
        self.utm_x_raw = data.data #offset전 utm_x값
    ...
    def utm_y_callback(self, data):
        self.utm_y_raw = data.data #offset전 utm_y값
    ...
    def heading_callback(self, data): #헤딩 좌표계를 UTM 좌표계로 변환
        self.yaw_raw = data.data #gps_mbc기준 헤딩값
    ...
    def boundery_alarm (self):
        API = Message(service_key = self.service_key)
        url = "https://kapi.kakao.com/v2/api/talk/memo/default/send"

        token = self.token # 변경된 토큰 대입
    ...
    def obs_alarm(self):
        API = Message(service_key = self.service_key) # API키 (성환) # API키 (우영)
        url = "https://kapi.kakao.com/v2/api/talk/memo/default/send"
        ...
        data = {
            "template_object": json.dumps({
                ...
                "buttons": [{"title": "차량의 상태를 확인해주세요","link": {}}]
            })
        }
        response = requests.post(url, headers=headers, data=data)
        print(response.status_code, ", 알림 발송")
